...
export const applyLevelOverride = (G) => {
  ...
  // React 19 strict mode freezes objects, use spread syntax to avoid mutation
  if (playerHandOverride.length > 0) {
    G.players[0].hand = [...G.players[0].hand, ...playerHandOverride];
  }
  if (enemyHandOverride.length > 0) {
    G.players[1].hand = [...G.players[1].hand, ...enemyHandOverride];
  }

  if (playerEffectsOverride.length > 0) {
    G.players[0].effects = [...G.players[0].effects, ...playerEffectsOverride];
  }
  if (enemyEffectsOverride.length > 0) {
    G.players[1].effects = [...G.players[1].effects, ...enemyEffectsOverride];
  }

  if (globalEffectsOverride.length > 0) {
    G.globalEffects = [...G.globalEffects, ...globalEffectsOverride];
  }
};
